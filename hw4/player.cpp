/******************************************************************************************************
 * Program: player.cpp
 * Author: Tu Lam
 * Date: 05/16/2019
 * Description: The implementation file to implement all the body function from class into the program.
 * Input: None.
 * Output: None.
 *****************************************************************************************************/

#include <iostream>
#include <string>
#include "event.h"
#include "player.h"

using namespace std;

/******************************************************
 * Function: Player::Player();
 * Input: Assigning the protected variables with value.
 * Output: None.
 *****************************************************/
Player::Player() {

	message = " ";
	id = '*';
}

/******************************************************
 * Function: void Player::percept();
 * Input: None.
 * Output: Print out the message for the Player class.
 *****************************************************/
void Player::percept() {

	cout << get_message() << endl;
}

/******************************************************
 * Function: void Player::set_id(char);
 * Input: None.
 * Output: Assign the variable to the member variable.
 *****************************************************/
void Player::set_id(char id) {

	this->id = id;
}

/******************************************************
 * Function: string Player::get_message();
 * Input: None.
 * Output: Return the value of the message variable.
 *****************************************************/
string Player::get_message() {

return message;
}

/******************************************************
 * Function: char Player::get_id();
 * Input: None.
 * Output: Return the value of the message variable.
 *****************************************************/
char Player::get_id() {

return id;
}
