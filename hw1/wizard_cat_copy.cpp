...
int main(int argc, char *argv[]) {
	...
	int read_file1 = 0, read_file2 = 0, wizard_size = 20, spellbook_size = 8;
	wizard wiz_info;
	...
	if (!wizard_db.is_open()) {
		cout << "File for 'wizards.txt' couldn't be open" << endl;
		exit(1);
	}
	wizard* wizard_data = create_wizard_db(wizard_size);
	...
	spellbook* book_data = create_spellbook_db(spellbook_size);
	spellbook_db >> read_file2;
	get_spellbook_db(book_data, read_file2, spellbook_db);
	spellbook_db.close();	

	title();
	wizard_login_info(wizard_data, wizard_size, &wiz_info);

	if (wiz_info.position_title == "Headmaster" || wiz_info.position_title == "Teacher") {
		faculty_display(book_data, spellbook_size, wizard_data);
	}

	else if (wiz_info.position_title == "Student") {
		student_display(book_data, spellbook_size, wizard_data);
	}
